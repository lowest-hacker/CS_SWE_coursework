...
class AVL(BST):
    ...
    def _remove_one_subtree(self, remove_parent: AVLNode, remove_node: AVLNode) ->\
        ...
        return: the lowest node in this transformation
        """
    ...
    def _remove_two_subtrees(self, parent: AVLNode, node: AVLNode) -> AVLNode:
        ...
        if s is not node.right:
            sp.left = s.right
            if s.right:
                s.right.parent = sp
            s.right = node.right

        # make sure parent pointers are correct
        if s and s.left:
            s.left.parent = s
        if s and s.right:
            s.right.parent = s

        bottom_node = sp.left if sp.left is not None else sp

        # handle case where successor becomes root
        if parent is None:
            self._root = s
            self._update_height(bottom_node)
            self._rebalance(bottom_node)
            return s
        # handle case where successor has valid parent
    ...
    def _rotate_left(self, node: AVLNode) -> AVLNode:
        ...
        :returns: the new root of the subtree originally rooted at ``node``
        """
    ...
    def _balance_factor(node: AVLNode) -> int:
        ...
        :param node: the node whose balance factor will be found
    ...
    def _update_height(self, node: AVLNode) -> None:
        ...
        :param node: the node from which to begin updating heights
        """
        while node is not None:
            node.height = _get_height(node)
            node = node.parent

    def _rebalance(self, node: AVLNode) -> None:
        """
        Rebalance the ``node`` subtree
        ...
        while node is not None:
            ...
            if _balance_factor(node) >= 2:
                # RL, RR rotation
                if _balance_factor(node.right) <= -1:
                    self._rotate_right(node.right)
                node = self._rotate_left(node)
            # node is left-heavy
            elif _balance_factor(node) <= -2:
                # LR, LL
                if _balance_factor(node.left) >= 1:
